...
| `seedTourPackages.js` | Create five sample tour packages from `seed-data/`   |
...
The tour fixtures live in `scripts/seed-data/`, one module per travel category,
so adding or reviewing sample data does not modify the database runner. The
runner is import-safe; it connects only when invoked directly with `node`.
