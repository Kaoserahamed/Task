...
describe('api/client', () => {
  ...
  test('turns non-2xx responses into ApiError with status, code and field errors', async () => {
    ...
    const error = await api.post('/user/auth/login', { email: 'a@b.c' }).catch((err) => err);
  ...
  test('auth:false omits the token even when one is stored', async () => {
    ...
    await api.post('/user/auth/login', { email: 'a@b.c' }, { auth: false });
  ...
  test('returns null for 204 and empty bodies', async () => {
    ...
    await expect(api.put('/user/auth/update', { name: 'Ada' })).resolves.toBeNull();
