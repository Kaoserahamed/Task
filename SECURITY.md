...
| Request validation covers the tour write endpoints only     | Everything else relies on Mongoose schema validation, which rejects unknown types but not extra fields                | Add a validator per resource under `backend/validators/`                                  |
