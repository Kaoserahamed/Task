...
| Request validation covers tour, company, and wishlist writes | Other resources still rely on Mongoose schema validation, which rejects unknown types but not extra fields            | Add a validator per remaining resource under `backend/validators/`                                   |
